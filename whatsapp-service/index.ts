  ...
  try {
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        ...
        // ========================================
        // EMIT AUTH FLOW EVENTS FOR CLIENT
        // ========================================
        // These events help debug authentication issues
        io.emit('account-status', { accountId, status: 'online' });
        io.emit('authenticated', { accountId });
        io.emit('connection-update', { accountId, status: 'open' });
        
        // Get user info and emit ready event
        ...
        if (user) {
          ...
          account.name = name;
          io.emit('account-info', { accountId, phoneNumber, name });
          io.emit('ready', { accountId, name });
          console.log('[AUTH] ✅ Ready event emitted for:', accountId, 'name:', name);
        }
      ...
      if (connection === 'close') {
        ...
        const TEMPORARY_CODES = [
          ...
          401, 408, 409, 429, 500, 502, 503, 504,
        ...
        io.emit('connection-update', { accountId, status: 'close', reason: errorMessage || `code: ${statusCode}` });
        ...
        if (isBanned) {
          // Emit banned event for client
          io.emit('banned', { accountId, reason: banReason });
          io.emit('auth-failure', { accountId, reason: banReason });
          await handleBannedAccount(accountId, banReason);
          return;
        }
        
        // Emit auth-failure for non-temporary errors
        if (!isTemporaryCode && statusCode !== 401) {
          io.emit('auth-failure', { accountId, reason: errorMessage || `Connection lost (code: ${statusCode})` });
        }
