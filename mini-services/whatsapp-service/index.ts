...
async function startSession(accountId: string, usePairingCode: boolean = false, phoneNumber?: string, forceNew: boolean = false) {
  ...
  try {
    ...
    // Save to database immediately (don't wait for sync)
    db.whatsAppAccount.upsert({
      where: { id: accountId },
      create: {
        id: accountId,
        ...
        autoResponsesSent: 0,
        isInActiveWindow: true,
        rateLimitHourCount: 0,
        rateLimitDayCount: 0,
        rateLimitHourReset: new Date(),
        rateLimitDayReset: new Date()
      },
      update: {
        status: 'connecting',
        lastSeen: new Date()
      ...
      console.log('[DB] ✅ Saved account to database:', accountId);
    }).catch(err => {
      console.error('[DB] ❌ Failed to save account to database:', accountId, err.message);
    });
...
// Load accounts from database and restore sessions
async function loadAccountsFromDatabase() {
  try {
    console.log('[DB] Loading accounts from database...');
    ...
    console.log(`[DB] Found ${dbAccounts.length} accounts in database`);
    
    for (const dbAccount of dbAccounts) {
      try {
        // Check if session exists in WhatsAppSession table
        const session = await db.whatsAppSession.findUnique({
          where: { accountId: dbAccount.id }
        });
        
        // Create account object in memory
        const account: Account = {
          id: dbAccount.id,
          phoneNumber: dbAccount.phoneNumber || undefined,
          name: dbAccount.name || undefined,
          profilePicture: dbAccount.profilePicture || undefined,
          status: 'offline', // Start as offline, will try to reconnect
          socket: null,
          lastSeen: dbAccount.lastSeen || undefined,
          warmingEnabled: dbAccount.warmingEnabled,
          warmingStats: {
            accountId: dbAccount.id,
            messagesReceived: dbAccount.messagesReceived,
            messagesSent: dbAccount.messagesSent,
            autoResponsesSent: dbAccount.autoResponsesSent,
            warmingStartTime: dbAccount.warmingStartTime || undefined,
            totalWarmingHours: 0,
            lastActivity: dbAccount.lastActivity || undefined,
            healthScore: dbAccount.healthScore,
            rateLimit: {
              messagesPerHour: config.maxMessagesPerHour,
              messagesPerDay: config.maxMessagesPerDay,
              currentHourCount: dbAccount.rateLimitHourCount,
              currentDayCount: dbAccount.rateLimitDayCount,
              lastHourReset: dbAccount.rateLimitHourReset,
              lastDayReset: dbAccount.rateLimitDayReset
            },
            currentPhase: dbAccount.currentPhase,
            warmingDays: dbAccount.warmingDays
          },
          warmingTimers: [],
          personality: dbAccount.personality ? {
            name: dbAccount.personality.name,
            age: dbAccount.personality.age,
            occupation: dbAccount.personality.occupation,
            location: dbAccount.personality.location,
            traits: JSON.parse(dbAccount.personality.traits || '[]'),
            writingStyle: dbAccount.personality.writingStyle,
            hobbies: JSON.parse(dbAccount.personality.hobbies || '[]'),
            responseStyle: dbAccount.personality.responseStyle,
            chronotype: (dbAccount.personality.chronotype as any) || 'regular',
            activeHoursStart: dbAccount.personality.activeHoursStart,
            activeHoursEnd: dbAccount.personality.activeHoursEnd,
            peakHours: JSON.parse(dbAccount.personality.peakHours || '[]'),
            avgResponseTime: dbAccount.personality.avgResponseTime,
            emojiUsage: (dbAccount.personality.emojiUsage as any) || 'moderate',
            avgMessageLength: (dbAccount.personality.avgMessageLength as any) || 'medium'
          } : null,
          pool: (dbAccount.pool as any) || 'offline',
          poolSince: dbAccount.poolSince,
          currentChatPartner: dbAccount.currentChatPartnerId || undefined,
          chatHistory: [],
          isInActiveWindow: dbAccount.isInActiveWindow,
          silentPeriod: { isActive: false },
          sessionActivity: { messagesInSession: 0 }
        };
        
        // Add to memory
        accounts.set(dbAccount.id, account);
        
        // Register personality if exists
        if (account.personality) {
          personalityRegistry.set(dbAccount.id, account.personality);
        }
        
        console.log(`[DB] Restored account: ${dbAccount.id} (${dbAccount.phoneNumber || 'no phone'})`);
        
        // If session exists, try to reconnect (with delay to avoid connection flood)
        if (session) {
          // Use setTimeout to stagger reconnections
          setTimeout(() => {
            console.log(`[DB] Attempting to reconnect session for: ${dbAccount.id}`);
            startSession(dbAccount.id, false, undefined, false).catch(err => {
              console.error(`[DB] Failed to reconnect ${dbAccount.id}:`, err.message);
            });
          }, Math.random() * 10000); // Random delay 0-10 seconds
        }
        
      } catch (err) {
        console.error(`[DB] Failed to restore account ${dbAccount.id}:`, err);
      }
    }
    
    console.log(`[DB] Restored ${accounts.size} accounts to memory`);
    
  } catch (error) {
    console.error('[DB] Failed to load accounts from database:', error);
  }
}
...
async function start() {
  try {
    ...
    await loadConfig();
    await loadAccountsFromDatabase(); // Load accounts after config
