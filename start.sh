...
mkdir -p /app/mini-services/whatsapp-service/sessions /app/mini-services/whatsapp-service/backups

# NOTE: DATABASE_URL is provided by Railway (PostgreSQL service)
...
if [ -n "$DATABASE_URL" ]; then
    # PostgreSQL: use db push (simpler, no migration files needed)
    echo "Using PostgreSQL database"
    bunx prisma db push --skip-generate --accept-data-loss 2>/dev/null || bunx prisma db push --skip-generate || echo "⚠️ DB push warning, continuing..."
else
...
if command -v unbuffer &> /dev/null; then
    unbuffer npx tsx index.ts 2>&1 | tee /app/mini-services/whatsapp-service/whatsapp-service.log &
elif command -v stdbuf &> /dev/null; then
    stdbuf -oL -eL npx tsx index.ts 2>&1 | tee /app/mini-services/whatsapp-service/whatsapp-service.log &
else
    npx tsx index.ts 2>&1 | tee /app/mini-services/whatsapp-service/whatsapp-service.log &
fi
WA_PID=$!
...
for i in $(seq 1 60); do
    ...
    if ! kill -0 $WA_PID 2>/dev/null; then
        ...
        tail -50 /app/mini-services/whatsapp-service/whatsapp-service.log 2>/dev/null || echo "No log available"
        echo "=== End of log ==="
        exit 1
    fi
...
npx tsx server.ts 2>&1 | tee /app/nextjs-server.log &
NEXT_PID=$!
...
for i in $(seq 1 60); do
    ...
    if ! kill -0 $NEXT_PID 2>/dev/null; then
        ...
        echo "=== Last 50 lines of log ==="
        tail -50 /app/nextjs-server.log 2>/dev/null || echo "No log available"
        echo "=== End of log ==="
        exit 1
    fi
...
echo ""
echo "Database: PostgreSQL (via Railway)"
echo ""
